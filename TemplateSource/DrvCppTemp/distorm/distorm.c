...
static void distorm_format_size(_WString* str, const _DInst* di, int opNum)
{
	...
	if (isSizingRequired)
	{
		switch (di->ops[opNum].size)
		{
			...
			case 8: strcat_WSN(str, "BYTE PTR "); break;
			case 16: strcat_WSN(str, "WORD PTR "); break;
			case 32: strcat_WSN(str, "DWORD PTR "); break;
			case 64: strcat_WSN(str, "QWORD PTR "); break;
			case 80: strcat_WSN(str, "TBYTE PTR "); break;
			case 128: strcat_WSN(str, "DQWORD PTR "); break;
			case 256: strcat_WSN(str, "YWORD PTR "); break;
	...
	for (i = 0; ((i < OPERANDS_NO) && (di->ops[i].type != O_NONE)); i++) {
		...
		switch (di->ops[i].type)
		{
			...
			case O_SMEM:
				...
				chrcat_WS(str, OPEN_CHR);
				if (_REGISTERS[di->ops[i].index].length == 3
					&& memcmp(_REGISTERS[di->ops[i].index].p, "RIP", 3) == 0)
				{
					int64_t tmpDisp64;
					tmpDisp64 = INSTRUCTION_GET_RIP_TARGET(di);
					str_code_hqw(str, (uint8_t*)&tmpDisp64);
				}
				else
				{
