...
namespace Detours
{
	namespace X86
	{
		...
		uint8_t *DetourFunction(uint8_t *Target, uint8_t *Detour, X86Option Options)
		{
			...
			uint8_t *jumpTrampolinePtr = (uint8_t *)ExAllocatePool(NonPagedPool,allocSize);
			...
			header->Random				= PsGetCurrentProcessId() + PsGetCurrentThreadId() + totalInstrSize;
			...
			// Apply any changes because of the different IP
			{
				...
				BYTE *data = Reassembler::Reassemble32((uint32_t)header->InstructionOffset, decodedInstructions, copyInsCount, &header->InstructionLength);
				if (!data)
				{
					ExFreePool(jumpTrampolinePtr);
					return nullptr;
				}
		...
		bool DetourRemove(uint8_t *Trampoline)
		{
			...
			if (!Internal::AtomicCopy4X8(header->CodeOffset, header->backup_code, header->backup_size))
				return false;
