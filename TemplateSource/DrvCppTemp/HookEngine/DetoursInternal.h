...
namespace Detours
{
	struct JumpTrampolineHeader
	{
		...
		size_t InstructionLength;	// Length of the instructions that were reassembled
		uint8_t *InstructionOffset;	// Where the new instructions are
		...
		size_t backup_size;        // Length of the instructions that were replaced
		uint8_t backup_code[0x100]; //Where the backup instructions are
