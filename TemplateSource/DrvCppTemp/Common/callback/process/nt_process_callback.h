...
{
	class nt_process_callback :public ddk::Singleton<nt_process_callback>
	{
	public:
		...
		nt_process_callback()
		{
			...
			auto p = ddk::util::get_proc_address("PsSetCreateProcessNotifyRoutineEx");
			if (p)
			{
				b_ex = true;
				auto ns = SAFE_NATIVE_CALL(PsSetCreateProcessNotifyRoutineEx, ddk::callback::nt_process_callback::_ProcessNotifyRoutineEx, BOOLEAN(FALSE));
				if (NT_SUCCESS(ns))
				{
					return;
				}
				LOG_DEBUG("failed PsSetCreateProcessNotifyRoutineEx\r\n");
			}
			{
				auto ns = PsSetCreateProcessNotifyRoutine(ddk::callback::nt_process_callback::_ProcessNotifyRoutine, FALSE);
				if (!NT_SUCCESS(ns))
				{
					LOG_DEBUG("failed PsSetCreateProcessNotifyRoutine\r\n");
		...
		~nt_process_callback()
		{
			...
			if (b_ex)
			{
				ddk::util::nt_syscall::getInstance().safeCall<fnPsSetCreateProcessNotifyRoutineEx>(
					"PsSetCreateProcessNotifyRoutineEx", ddk::callback::nt_process_callback::_ProcessNotifyRoutineEx, BOOLEAN(TRUE));
			}
			{
				PsSetCreateProcessNotifyRoutine(ddk::callback::nt_process_callback::_ProcessNotifyRoutine, TRUE);
			}
		...
		static VOID _ProcessNotifyRoutine(
			...
			IN BOOLEAN  Create)
		{
			ddk::callback::nt_process_callback::getInstance().do_callback(ParentId, ProcessId, Create);
		}
		void do_callback(
			IN HANDLE  ParentId,
			IN HANDLE  ProcessId,
		...
		{
			ddk::callback::nt_process_callback::getInstance().do_callback_ex(Process, ProcessId, CreateInfo);
		}
		void do_callback_ex(
			_Inout_  PEPROCESS              Process,
			_In_     HANDLE                 ProcessId,
