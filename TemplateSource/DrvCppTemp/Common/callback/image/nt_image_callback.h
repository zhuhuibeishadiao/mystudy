...
{
	//Win10以後的系統上可以用SeRegisterImageVerificationCallback來過濾更神奇的
	
	class nt_image_callback :public ddk::Singleton<nt_image_callback>
	{
	public:
		using nt_image_callback_type = std::function<void(PUNICODE_STRING, HANDLE, PIMAGE_INFO)>;
		nt_image_callback() {
			ExInitializeRundownProtection(&run_for);
			PsSetLoadImageNotifyRoutine(ddk::callback::nt_image_callback::_ImageLoadNotifyRoutine);
		}
		~nt_image_callback() {
			PsRemoveLoadImageNotifyRoutine(ddk::callback::nt_image_callback::_ImageLoadNotifyRoutine);
		...
		{
			ddk::callback::nt_image_callback::getInstance().do_callback(FullImageName, ProcessId, ImageInfo);
		}
		void do_callback(
			__in_opt PUNICODE_STRING  FullImageName,
			__in HANDLE  ProcessId,
