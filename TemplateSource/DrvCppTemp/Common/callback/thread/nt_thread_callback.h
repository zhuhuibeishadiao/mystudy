...
{
	//PsSetLegoNotifyRoutine 只能有一個,PspExitThread時才調用

	class nt_thread_callback :public ddk::Singleton<nt_thread_callback>
	{
	public:
		...
		nt_thread_callback()
		{
			...
			PsSetCreateThreadNotifyRoutine(ddk::callback::nt_thread_callback::_CreateThreadNotifyRoutine);
		}
		~nt_thread_callback()
		{
			PsRemoveCreateThreadNotifyRoutine(ddk::callback::nt_thread_callback::_CreateThreadNotifyRoutine);
		...
		static VOID _CreateThreadNotifyRoutine(
			...
			_In_ BOOLEAN Create)
		{
			ddk::callback::nt_thread_callback::getInstance().do_callback(ProcessId, ThreadId, Create);
		}
		void do_callback(
			_In_ HANDLE ProcessId,
			_In_ HANDLE ThreadId,
