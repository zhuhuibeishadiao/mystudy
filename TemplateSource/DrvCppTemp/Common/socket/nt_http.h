...
namespace ddk
{
	#define MOZZILA_USER_AGENT "Mozilla / 5.0 (Windows NT 6.3; WOW64) AppleWebKit / 537.36 (KHTML, like Gecko) Chrome / 41.0.2272.118 Safari / 537.36"

	#define MS_SYMBOL_SERVER_USER_AGENT "Microsoft-Symbol-Server"

	#define MS_SYMBOL_SERVER "msdl.microsoft.com"

	#define MS_SYMBOL_SERVER_DOWNLOAD_URI "/download/symbols"

	#define MAX_HTTP_URI_LEN 256
	...
	public:
		...
		void Get(ddk::wsk::tcp_client *pClient)
		{
			m_Pos = 0;
			TotalSize = 0;
			while (1)
		...
		void Parse()
		{
			...
			while (1)
			{
				...
				UNREFERENCED_PARAMETER(line_size);
				m_Content = m_Pos;
				//200 OK
				//LOG_DEBUG("Parse %s\r\n", line.c_str());
			...
			{
				_content_size = TotalSize - m_Content;
			}
	...
	public:
		...
		bool get(std::string uri, http_response &httpResponse)
		{
			...
			auto exit_1 = std::experimental::make_scope_exit([&]() {if (send_buff)free(send_buff); });
			if (send_buff)
			{
				auto sendSize = (ULONG)s_requset.length();
				ULONG sent = 0;
