...
namespace ddk::ntos
{
	class nt_object_types :public ddk::Singleton<nt_object_types>
	{
	public:
		nt_object_types() {
			m_init = false;
		...
		void print_all()
		{
			...
			{
				count++;
				LOG_DEBUG("object_type %ws,%p\r\n", info.first.c_str(), info.second);
			}
			LOG_DEBUG("object type count=%d\r\n", count);
		}
	private:
		void take_snapshot()
		{
				...
				if (NT_SUCCESS(ns))
				{
					...
					for (auto Bucket = 0; Bucket < 37; Bucket++)
					{
						...
						while (DirectoryEntry != NULL)
						{
							...
							ns = ObQueryNameString(DirectoryEntry->Object,
