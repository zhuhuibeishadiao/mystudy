...
/***************************************获得进程全路径*********************************************/
NTSTATUS NTAPI ZwQueryInformationProcess(__in HANDLE ProcessHandle,
										 __in PROCESSINFOCLASS ProcessInformationClass,
										 __out_bcount(ProcessInformationLength) PVOID ProcessInformation,
										 __in ULONG ProcessInformationLength,
										 __out_opt PULONG ReturnLength);

NTSTATUS  FGGetProcessFullNameByPid(HANDLE nPid, PUNICODE_STRING  FullPath)
{
...
/***************************************设备名与连接名的转换****************************************/
static BOOLEAN QueryVolumeName(WCHAR ch, WCHAR * name, USHORT size)
{
	...
	ntStatus = FGGetSymbolicVolume(&LinkName, &VolName);
...
NTSTATUS FGGetDosVolume(IN PUNICODE_STRING DeviceName, OUT PUNICODE_STRING DosName)
{
	...
	for (c = L'A'; c <= L'Z'; c++)
	{
		...
		status = FGGetSymbolicVolume(&driveLetterName, &linkTarget);
		if (!NT_SUCCESS(status))
		{
			continue;
		}
...
BOOLEAN FGGetLinkName(WCHAR *wszNTName, WCHAR *wszFileName)
{
	...
	if (!NT_SUCCESS(FGGetDosVolume(&ustrDeviceName, &ustrDosName)))
	{
		return FALSE;
	}
...
BOOLEAN FGGeDosPath(WCHAR * filename, WCHAR * ntname)
{
...
BOOL FGConverShortToLongPath(WCHAR *wszLongName, WCHAR *wszShortName, ULONG size)
{
	WCHAR			*szResult = NULL;
