...
CSliderDemoDlg::CSliderDemoDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(IDD_SLIDERDEMO_DIALOG, pParent)
{
...
BEGIN_MESSAGE_MAP(CSliderDemoDlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_WM_HSCROLL()
...
void CSliderDemoDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
void CSliderDemoDlg::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	auto index = m_slider.GetPos();
	CString str;
	str.Format(L"当前位置：%d", index);

	SetDlgItemTextW(IDC_STATIC_OUTPUT, str);
