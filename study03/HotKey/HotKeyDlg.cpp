...
BEGIN_MESSAGE_MAP(CHotKeyDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BUTTON_SETHOTKEY, &CHotKeyDlg::OnBnClickedButtonSethotkey)
END_MESSAGE_MAP()
...
void CHotKeyDlg::OnBnClickedButtonSethotkey()
{
	// TODO: 在此添加控件通知处理程序代码
	WORD virtualKey;
	WORD modifier;
	m_hotkey.GetHotKey(virtualKey, modifier);
	if (RegisterHotKey(m_hWnd, ))
	{
	}
}
